/**
 * PushApp Flutter SDK — documentation site
...
function localMarkdownUrl(repoPath) {
  return `../${repoPath}`;
}
...
async function fetchMarkdown(repoPath) {
  const urls = isLocalDev()
    ? [localMarkdownUrl(repoPath), rawMarkdownUrl(repoPath)]
    : [rawMarkdownUrl(repoPath)];
  ...
  for (const url of urls) {
    try {
      ...
      if (!res.ok) {
        lastError = new Error(`Could not load ${repoPath} (${res.status})`);
        continue;
      }
      return res.text();
    } catch (err) {
  ...
  throw lastError || new Error(`Could not load ${repoPath}`);
}
